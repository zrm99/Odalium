...
const connection = `postgres://${process.env.DB_USERNAME}:\
${process.env.DB_PASSWORD}@${process.env.PG_HOST}:\
${process.env.PG_PORT}/${process.env.DB_NAME}`;
...
module.exports = {
  ...
  insertUser: async function insertUser(req, date) {
  ...
  checkExistingUser: async function checkExistingUser(username) {
    let query = 'SELECT "username" FROM t_users WHERE "username" = $1';
    let queryValues = [username];

    return client.query(query, queryValues);
  },

  findUsernameByIp: async function findUsernameByIp(ipAddress) {
  ...
  insertMiniverseTopic: async function insertMiniverseTopic(req, topicDateCreated, topicID) {
  ...
  createUserReply: async function createUserReply(req, replyID) {
  ...
  insertProfilePost: async function insertProfilePost(req, profilePostCreationDate, postID) {
